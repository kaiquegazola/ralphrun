...
const PREAMBLE = [
  ...
  " status, deps[], retries, description, acceptance[], scope[], shared_scope?, forbidden_scope?, scope_requests?, parallel, resources, verify, required_host?}]}",
  ...
  "required_host is optional: use a Node platform id (or a non-empty list) only when implementation/verify truly requires that host; omit it for portable or cross-platform work.",
  "Do not put one host on an integration/release gate that requires evidence from multiple platforms; keep those gates cross-platform.",
  ...
  "shared_scope: paths or globs that may be edited by multiple tasks, but those tasks",
  "are serialized at runtime. forbidden_scope: paths or globs this task must never",
  "edit, even if another scope allows them; forbidden takes precedence.",
  "scope_requests: preserve reviewer-generated path/reason objects that record a",
  "plan-level scope change still needed; do not silently drop them during an update.",
